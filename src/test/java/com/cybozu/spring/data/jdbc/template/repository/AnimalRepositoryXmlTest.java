...
public class AnimalRepositoryXmlTest {
    private static final Operation DROP_TABLE = Operations.sql("DROP TABLE IF EXISTS " + Animal.TABLE_NAME);
    private static final Operation CREATE_TABLE = Operations.sql("CREATE TABLE " + Animal.TABLE_NAME + " ("
    ...
    public void testGetByNames() {
        initDb(insertBuilder()
                .values("serval", "Leptailurus serval", "LC")
                .values("crested ibis", "Nipponia nippon", "EN")
                .build());
    ...
    public void testUpdate() {
        ...
        Assertions.assertThat(table()).hasNumberOfRows(1).row(0).hasValues(1L, "humboldt penguin",
                "Spheniscus humboldti", "VU");
    }
